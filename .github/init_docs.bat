...
powershell -Command "(Get-Content 'conf.py') | ForEach-Object { if ($_ -match '^extensions =') { 'extensions = [\"sphinx.ext.todo\", \"sphinx.ext.viewcode\", \"sphinx.ext.autodoc\"]' } else { $_ } } | Set-Content 'conf.py'" || (echo !RED!BOLD FAILED: Couldn't modify config.py !RESET! & exit /b 1)
echo !GREEN! SUCCESSFUL [4/5]: Modified extensions in config.py !RESET!

:: Modify theme in config.py
powershell -Command "(Get-Content 'conf.py') | ForEach-Object { if ($_ -match '^html_theme =') { 'html_theme = \"sphinx_rtd_theme\"' } else { $_ } } | Set-Content 'conf.py'" || (echo !RED!BOLD FAILED: Couldn't modify theme in config.py !RESET! & exit /b 1)
